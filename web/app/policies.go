...
func getRedisStat(key string) string { val, err := database.Rdb.Get(database.Ctx, key).Result(); if err != nil || val == "" { return "0" }; return val }
...
func savePolicies(w http.ResponseWriter, r *http.Request) {
	...
	database.DB.Exec("UPDATE log_policies SET enabled=$1, minimum_severity=$2, ignored_apps=$3, ignored_hosts=$4, ignored_keywords=$5 WHERE id=1", enabled, r.FormValue("min_severity"), r.FormValue("apps"), r.FormValue("hosts"), r.FormValue("keywords"))
	database.SyncPolicyToRedis()
